...
class UsGaapStore:
    ...
    def ingest_us_gaap_csvs(
        ...
        try:
            while True:
                ...
                for cell in row.entries:
                    ...
                    triplet_key_bytes = (
                        encode_string_to_bytes(cell.concept)
                        + encode_string_to_bytes(cell.uom)
                        + encode_float_to_raw_bytes(cell.value)
                    )
        ...
        self.data_store.write(
            b"__triplet_count__",
            encode_u32_to_raw_bytes(total_triplets)
        )
        ...
        self.data_store.write(
            b"__pair_count__", encode_u32_to_raw_bytes(total_pairs)
        )
        ...
        for pair, i_cells in tqdm(
            ...
            for i in range(0, len(keys_for_pair), READ_BATCH_SIZE):
                ...
                for raw_bytes, key in zip(raw_bytes_list, chunk_keys):
                    ...
                    all_values_for_pair.append(decode_float_from_bytes(raw_bytes))
            ...
            scaler_bytes_encoded = encode_joblib_object_to_bytes(scaler)
            pair_id = self._get_pair_id(pair)
            self.data_store.write(
                SCALER_NAMESPACE.namespace(encode_u32_to_raw_bytes(pair_id)),
            ...
            for i_cell, scaled_val in zip(i_cells, scaled_vals):
                scaled_val_key = SCALED_SEQUENTIAL_CELL_NAMESPACE.namespace(
                    encode_u32_to_raw_bytes(i_cell)
                )
                scaled_val_bytes = encode_float_to_raw_bytes(scaled_val)
    ...
    def generate_pca_embeddings(self):
        ...
        pca_embedding_entries = [
            (
                PCA_REDUCED_EMBEDDING_NAMESPACE.namespace(
                    encode_u32_to_raw_bytes(pair_id)
                ),
                # IMPORTANT: `float64` type MUST be used here as the PCA embeddings are encoded as float64
                encode_numpy_array_to_raw_bytes(vec, np.float64),  # Encode numpy array directly
            )
        ...
        pca_model_bytes_encoded = encode_joblib_object_to_bytes(pca)
        self.data_store.write(
    ...
    def load_pca_model(self) -> Optional[PCA]:
        ...
        if pca_model_bytes is None:
            return None
        return decode_joblib_object_from_bytes(pca_model_bytes)
    ...
    def get_embedding_matrix(self) -> Tuple[np.ndarray, list]:
        ...
        total_pairs =decode_u32_from_raw_bytes(raw_bytes)
        ...
        for pair_id in range(total_pairs):
            ...
            concept, offset = decode_string_from_bytes(pair_bytes, 0)
            uom, _ = decode_string_from_bytes(pair_bytes, offset)
    ...
    def iterate_concept_unit_pairs(self) -> Iterator[Tuple[int, ConceptUnitPair]]:
        ...
        for pair_id in range(total_pairs):
            ...
            pair_mv = pair_bytes
            concept, offset = decode_string_from_bytes(pair_mv, 0)
            uom, _ = decode_string_from_bytes(pair_mv, offset)
    ...
    def batch_lookup_by_indices(self, cell_indices: list[int]) -> list[FullCellData]:
        ...
        for i_cell, result in zip(cell_indices, step1_results):
            ...
            pair_id = decode_u32_from_raw_bytes(meta_bytes)
        ...
        for i_cell, s1_result in zip(cell_indices, step1_results):
            ...
            scaler_key = SCALER_NAMESPACE.namespace(encode_u32_to_raw_bytes(pair_id))
            if scaler_key in self._scaler_cache:
                scaler = self._scaler_cache[scaler_key]
            else:
                scaler_bytes = s2_result.get("scaler")
                scaler = (
                    decode_joblib_object_from_bytes(scaler_bytes)
                    if scaler_bytes
                    else None
                )
    ...
    def cache_stage1_inference_batch(self, batch: list[Stage1InferenceRecord]) -> None:
        ...
        for record in batch:
            key_bytes = STAGE1_LATENT_EMBEDDING_NAMESPACE.namespace(encode_u32_to_raw_bytes(record.i_cell))
            latent_bytes = encode_numpy_array_to_raw_bytes(record.latent_embedding, np.float32)
    ...
    def get_stage1_latent_matrix_from_indices(self, cell_indices: list[int]) -> np.ndarray:
        read_keys = [
            STAGE1_LATENT_EMBEDDING_NAMESPACE.namespace(encode_u32_to_raw_bytes(i))
            for i in cell_indices
        ]
        raw = self.data_store.batch_read(read_keys)
        vecs = [decode_numpy_array_from_bytes(b, np.float32) for b in raw]
    ...
    def get_stage1_latent_cell_indices_from_triplets(self, triplets: list[UsGaapCsvTriplet]) -> list[int]:
        ...
        cell_indices = [
            decode_u32_from_raw_bytes(i_cell_bytes)
