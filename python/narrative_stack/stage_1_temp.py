...
class ConceptValueDataset(Dataset):
    ...
    def __init__(
        self, us_gaap_store: UsGaapStore, device: torch.device, return_scaler=False
    ):
        # self.rows = scaled_tuples
        # self.lookup = embedding_lookup
        self.device = device
        # self.scalers = scalers
        self.us_gaap_store = us_gaap_store
        self.return_scaler = return_scaler
        self.triplet_count = self.us_gaap_store.get_triplet_count()
    ...
    def __getitem__(self, idx):
        ...
        #     embedding = np.array(embedding, dtype=np.float32)  # shape (234,)

        value = item_data["scaled_value"]
        # value = np.array([item_data["scaled_value"]], dtype=np.float32)  # shape (1,)

        x = torch.tensor(
            np.concatenate([embedding, [value]]), dtype=torch.float32, device=self.device
        )
        y = x.clone()
...
# )
train_loader = DataLoader(
    ConceptValueDataset(us_gaap_store, device=device, return_scaler=True),
    batch_size=batch_size,
    shuffle=True,
    collate_fn=collate_with_scaler,
    num_workers=0,  # SIMD R Drive's Python bindings are currently not thread-safe
)
